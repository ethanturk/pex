...
async fn apply_finding_guards_with_relocation(
    ...
    for mut f in aggregate.findings.drain(..) {
        ...
        match relocate_anchor(
            provider, permits, file_path, new_content, &f, retry_count, meter, diag,
        )
        .await
        {
            Ok(Some(snippet)) => {
...
async fn process_file(idx: usize, ctx: Arc<ReviewRunCtx>) -> Result<FileOutcome, AppError> {
    ...
    let mut hunk_results: Vec<(usize, Result<String, AppError>)> =
        Vec::with_capacity(total_hunks);
    {
        let spawn_hunk =
            |js: &mut tokio::task::JoinSet<(usize, Result<String, AppError>)>, hunk_idx: usize| {
    ...
    for (hunk_idx, result) in hunk_results {
        match result {
            ...
            Err(e) => {
                ...
                file_findings.push((hunk_idx + 1, format!("[skipped — error: {}]", e)));
            }
        }
    }
...
pub async fn run_review(
    ...
    if !pending.is_empty() {
        ...
        while let Some(joined) = joinset.join_next().await {
            ...
            let outcome = joined
                .map_err(|e| AppError::Ai(format!("File review task failed: {}", e)))??;
...
pub async fn review_single_file(
    ...
    let specialist_prompts: Arc<Vec<SpecialistPrompt>> = Arc::new(if mode == ReviewMode::Thorough {
        PromptKey::THOROUGH_SPECIALISTS
            .iter()
